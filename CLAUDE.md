...
**Mission**: Build a complete, production-ready skill package for PDF.js (Mozilla) and publish it under the OpenAEC Foundation on GitHub. This is your standing order for every session in this workspace.
...
**End state**: A published GitHub repo at `https://github.com/OpenAEC-Foundation/PDFjs-Claude-Skill-Package` with:
...
- Repository topics set (claude, skills, pdfjs, pdf, document-viewer, ai, deterministic, openaec)
...
| docs/masterplan/pdfjs-masterplan.md | Planning | Execution plan with phases, prompts, dependencies |
...
|----------|---------|--------|
| syntax/ | API syntax, code patterns, method signatures | pdfjs-syntax-{topic} |
| impl/ | Development workflows, integration guides | pdfjs-impl-{topic} |
| errors/ | Error handling patterns, debugging | pdfjs-errors-{topic} |
| core/ | Cross-cutting concerns, architecture | pdfjs-core-{topic} |
| agents/ | Intelligent orchestration | pdfjs-{agent-name} |
...
├── docs/
│   ├── masterplan/              # pdfjs-masterplan.md
│   └── research/                # vooronderzoek-pdfjs.md, topic-research/, fragments/
└── skills/
    └── source/
        ├── pdfjs-syntax/        # Viewer API, worker API, rendering, text layer
        ├── pdfjs-impl/          # Custom viewer, annotations, search, printing
        ├── pdfjs-errors/        # Worker errors, rendering issues, font problems
        ├── pdfjs-core/          # Architecture, rendering pipeline, worker model
        └── pdfjs-agents/        # Validation, code generation agents
```
Single technology package — all skills share the `pdfjs-` prefix.
...
5. **Read docs/masterplan/pdfjs-masterplan.md** → Know the execution plan and current phase details
...
6. TypeScript/JavaScript code examples with proper imports
...
- Vooronderzoek: `docs/research/vooronderzoek-pdfjs.md`
...
- Naming: `pdfjs-{category}-{topic}`
- TypeScript/JavaScript examples with proper imports from pdfjs-dist
